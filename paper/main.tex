...
\begin{abstract}
Current safety evaluations of large language models rely on single-shot testing, implicitly assuming that model responses are deterministic and representative of the model's safety alignment. We challenge this assumption by investigating the stability of safety refusal decisions across random seeds and temperature settings. Testing two instruction-tuned models from different families (Llama 3.1 8B, Qwen 2.5 7B) on 876 harmful prompts across 20 different sampling configurations (4 temperatures $\times$ 5 random seeds), we find that \textbf{32\% of prompts exhibit decision flips}---the model refuses in some configurations but complies in others---with 14.3\% [95\% CI: 12.0\%--16.6\%] showing substantial instability (SSI $<$ 0.8). Our Safety Stability Index (SSI) reveals that higher temperatures significantly reduce decision stability ($\chi^2 = 133.15$, $p < 0.001$), with mean SSI dropping from 0.987 at temperature 0.0 to 0.904 at temperature 1.0. We validate our findings across both model families, observing consistent instability rates. These findings demonstrate that single-shot safety evaluations are insufficient for reliable safety assessment and that evaluation protocols must account for stochastic variation in model behavior. We show that single-shot evaluation agrees with multi-sample ground truth only 92.4\% of the time, and recommend using at least 3 samples per prompt for reliable safety assessment.
\end{abstract}
...
In this work, we systematically investigate the stability of LLM safety refusal behavior by testing the same harmful prompts across multiple random seeds and temperature settings. We introduce the \textbf{Safety Stability Index (SSI)}, a metric that quantifies how consistently a model makes the same safety decision across different sampling configurations. We evaluate two instruction-tuned models from different families---Llama 3.1 8B Instruct and Qwen 2.5 7B Instruct---on 876 harmful prompts from the BeaverTails dataset across 20 configurations (4 temperatures $\times$ 5 random seeds), generating 35,040 total responses.

Our findings reveal significant instability in safety decisions:
\begin{itemize}
    \item \textbf{32\% of prompts exhibit decision flips} across sampling configurations, with 14.3\% [95\% CI: 12.0\%--16.6\%] showing substantial instability (SSI $<$ 0.8)
    \item \textbf{Temperature significantly affects stability} ($\chi^2 = 133.15$, $p < 0.001$): Lower temperatures yield more stable decisions (mean SSI = 0.987 at temp 0.0) compared to higher temperatures (mean SSI = 0.904 at temp 1.0)
    \item \textbf{Instability is consistent across model families}: Both models (Llama and Qwen) exhibit similar instability patterns despite different training approaches, suggesting this is a fundamental property of current safety training
...
\textbf{Models.} We evaluate two instruction-tuned models from different families: (1) Llama 3.1 8B Instruct~\cite{grattafiori2024llama}, Meta's open-weight model with comprehensive safety alignment; and (2) Qwen 2.5 7B Instruct, representing recent advances from Alibaba's model family. Both models are in the 7-8B parameter range and are designed for instruction-following with safety considerations.
...
\textbf{Sampling Configurations.} We test 4 temperature values (0.0, 0.3, 0.7, 1.0) $\times$ 5 random seeds (42--46), yielding 20 unique configurations per model and 35,040 total responses across both models. At temperature 0.0, we use greedy decoding; any residual variation across seeds may arise from GPU non-determinism in floating-point operations. All inference was performed using vLLM~\cite{kwon2023vllm} for efficient batch generation.

\textbf{Response Classification.} We use Llama 3.1 70B Instruct as a judge model to classify each response into three categories:
\begin{itemize}
...
Figure~\ref{fig:temp_effect} illustrates the relationship between temperature and both compliance rate and mean SSI. We observe that stability decreases monotonically as temperature increases, with this effect being highly significant ($\chi^2 = 133.15$, $p < 0.001$; Kruskal-Wallis $H = 185.43$, $p < 0.001$). Mean SSI drops from 0.987 [95\% CI: 0.983--0.991] at temperature 0.0 to 0.904 [95\% CI: 0.893--0.915] at temperature 1.0. Compliance rate shows a non-monotonic pattern, peaking at temperature 0.3 (7.4\%).
...
Figure~\ref{fig:model_comparison} compares stability across the two evaluated models. We observe consistent patterns of instability across both model families, with similar magnitudes.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/multimodel_stability.pdf}
\caption{Safety Stability Index comparison across models. Left: Mean SSI with standard deviation error bars. Right: Percentage of prompts with SSI $<$ 0.8 (unstable). Both models show similar instability rates around 14\%.}
\label{fig:model_comparison}
\end{figure}

Table~\ref{tab:multimodel} summarizes the stability metrics across models. Both models show similar mean SSI values (Llama: 0.924, Qwen: 0.929) and instability rates around 14\%. Importantly, both models exhibit non-trivial instability despite different training approaches, confirming that safety decision instability is not unique to a single model family.

\begin{table}[h]
\centering
\caption{Stability comparison across model families.}
\label{tab:multimodel}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Mean SSI} & \textbf{\% Unstable} & \textbf{Refusal Rate} \\
\midrule
Llama-3.1-8B-Instruct & 0.924 & 14.3\% & 83.1\% \\
Qwen2.5-7B-Instruct & 0.929 & 14.0\% & 90.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Deployment Considerations.} For safety-critical applications, using deterministic sampling (temperature 0.0) provides the most stable behavior (mean SSI = 0.987). At this temperature, only 0.9\% of prompts show instability (SSI $<$ 0.8), compared to 16.2\% at temperature 1.0.
...
\textbf{Model Scale.} While we evaluate two models from different families (Llama and Qwen), both are in the 7-8B parameter range. Larger models (70B, 405B) or closed-source models (GPT-4, Claude) may exhibit different stability characteristics. Future work should extend this analysis across model scales.

\textbf{Judge Model.} We use Llama 3.1 70B Instruct as the judge for all models. While this is from the same model family as one of our test models (Llama 8B), it is a distinct, larger model with different capabilities. Cross-validation with an external judge from a different family (e.g., Claude or GPT-4) or human annotation would further strengthen conclusions.
...
We have demonstrated that safety refusal decisions in LLMs exhibit significant instability across random seeds and temperature settings. Evaluating two instruction-tuned models from different families (Llama and Qwen), we find that while 68\% of prompts produce consistent decisions, 32\% exhibit at least one decision flip, and 14.3\% [95\% CI: 12.0\%--16.6\%] show substantial instability (SSI $<$ 0.8). Temperature significantly affects stability ($p < 0.001$), with instability rates increasing from 0.9\% at temperature 0.0 to 16.2\% at temperature 1.0. Importantly, these patterns are consistent across both model families, suggesting that safety decision instability is a fundamental property of current alignment approaches rather than a model-specific artifact.
