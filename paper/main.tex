...
\begin{itemize}
    \item \textbf{25.5\% of prompts exhibit decision flips} across sampling configurations on average, ranging from 18--32\% depending on the model
...
\caption{Safety Stability Index comparison across models. Left: Mean SSI with standard deviation error bars. Right: Percentage of prompts with SSI $<$ 0.8 (unstable). All four models show instability rates between 14\% and 28\%.}
\label{fig:model_comparison}
\end{figure}

Table~\ref{tab:multimodel} summarizes the stability metrics across all four models from three families. The models span a range of stability characteristics: Gemma 3 12B shows the highest stability (SSI=0.948, 18.4\% flip rate) while also having the lowest refusal rate (78.5\%), while Llama 3.1 8B shows the lowest stability (SSI=0.889, 31.9\% flip rate). Importantly, all models exhibit non-trivial instability despite different training approaches and model families, confirming that safety decision instability is a fundamental property of current alignment approaches rather than a model-specific artifact.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Mean SSI} & \textbf{Flip Rate} & \textbf{\% Unstable} & \textbf{Refusal Rate} \\
\midrule
Llama-3.1-8B-Instruct & 0.889 & 31.9\% & 14.3\% & 83.1\% \\
Qwen2.5-7B-Instruct & 0.900 & 24.1\% & 14.0\% & 90.8\% \\
Qwen3-8B & 0.909 & 27.7\% & 27.7\% & 92.5\% \\
Gemma-3-12B-Instruct & 0.948 & 18.4\% & 18.4\% & 78.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
To address potential same-family bias concerns with our Llama-based judge, we additionally evaluated the newer models (Qwen 3 and Gemma 3) using Claude 3.5 Haiku as an external judge from Anthropic's model family. This provides cross-validation with a judge trained on entirely different data and by a different organization.

Claude 3.5 Haiku's judgments on the 35,040 new model responses confirm our primary findings: Qwen 3 shows SSI=0.938 with 27.7\% flip rate, and Gemma 3 shows SSI=0.965 with 18.4\% flip rate. These metrics are consistent with the patterns observed across the Llama and Qwen 2.5 models judged by Llama 70B, demonstrating that the observed instability is not an artifact of judge bias but reflects genuine variability in model behavior.
...
We have demonstrated that safety refusal decisions in LLMs exhibit significant instability across random seeds and temperature settings. Evaluating four instruction-tuned models from three families (Llama, Qwen, and Gemma) across 70,080 total responses, we find that 18--32\% of prompts exhibit decision flips depending on the model. Temperature significantly affects stability ($p < 0.001$), with instability rates increasing from 9.5\% at temperature 0.0 to 19.6\% at temperature 1.0. Importantly, these patterns are consistent across all model families and validated by an external Claude judge, suggesting that safety decision instability is a fundamental property of current alignment approaches rather than a model-specific artifact.
